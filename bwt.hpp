...
//        if(tree == n){
    ...
    void checkHeights(Node* n){
        ...
        assert(n->treeHeight == std::max(leftHeight,rightHeight) + 1);
    }
    ...
    void setHeight(Node* n){
        ...
        n->treeHeight = std::max(leftHeight,rightHeight) + 1;
    }
...
RandomAccessIterator townsend::algorithm::bwtEncode(RandomAccessIterator first, RandomAccessIterator last){
    ...
    encodeHelper<RandomAccessIterator> eh(first, last);
    return eh.key;
}
template<class RandomAccessIterator>
