...
//
//std::string compress_block_old(std::string text, size_t block_size, size_t markov_order) {
////    EASY_FUNCTION();
...
//    EASY_FUNCTION();
    ...
    size_t L1=L;
    std::vector<std::string> keys_stack;
//    std::map<std::string, std::vector<size_t>> keys_stack_map;
...
//    Counts<std::string> keys_copy = get_keys_by_lcp(text, 1, ALPHA_LIMIT, block_size, block_size);
    std::map<std::string, std::vector<size_t>> keys_copy = get_keys_positions_by_lcp(text, 1, ALPHA_LIMIT, block_size, block_size);
//    std::map<std::string, std::unordered_set<size_t>> keys_copy = get_keys_positions_by_lcp(text, 1, ALPHA_LIMIT, block_size, block_size);
...
//    std::map<std::string, std::vector<size_t>> keys;
    std::map<std::string, std::unordered_set<size_t>> keys;
    std::map<size_t, size_t> keys_final_positions; // pair of position and key length

    float limweight = 0;//get_weight2("--", 2, L1, bi, B);

    for(const auto& i: keys_copy) {
        if(i.first.size()>=ALPHA_LIMIT && i.second.size()>=2 && get_weight2(i.first, i.second.size(), L1, bi, B)>limweight) {
            keys[i.first] = std::unordered_set<size_t>(i.second.begin(), i.second.end());
        }
    }

    std::vector<size_t> max_keys_lens(keys_copy[""].begin(), keys_copy[""].end());
    keys_copy.clear();
...
//        max_weight = -1.0;
        ...
        key2 = sorted_by_weights.back().first;
        std::cout<<"\nkey2= <"<<key2<<"> "<<keys[key2].size()<<std::endl;
        auto key2_count = keys[key2].size();
        sorted_by_weights.pop_back();

        for(const auto& i: keys[key2]) {
//            keys_final_positions[i] = keys_stack.size();
            keys_final_positions[i] = key2.size();
        }

        for(const auto& pos: std::unordered_set<size_t>(keys[key2].begin(), keys[key2].end())) {
//        for(auto m=keys[key2].begin(); m!=keys[key2].end(); m++) {
//            auto pos = *m;
//            std::cout<<"pos= "<<pos<<std::endl;

            for(auto i=1; i<=max_keys_lens[pos]; i++) {
//                if (i != key2.size()) {
                    auto s1 = text.substr(pos, i);
                    if(keys.count(s1)) {
                        keys[s1].erase(pos);
                    }
//                }
                for(auto j=1; j<=max_keys_lens[pos+i]; j++) {
                    auto s2 = text.substr(pos+i, j);
                    if(keys.count(s2)) {
                        keys[s2].erase(pos+i);
                    }
                }
            ...
            int k = pos;
            while(k>0 && max_keys_lens[k-1]>pos-k) {
                for(auto i=1; i<=max_keys_lens[k]; i++) {
                    auto s1 = text.substr(k, i);
                    if(keys.count(s1)) {
//                        keys[text.substr(k, i)].erase(k);
                        keys[text.substr(k, i)].erase(k);
                    }
                }
                k--;
...
//        keys_copy[key2] = keys[key2];

        keys.erase(key2);

        std::cout<<"keys.size() before: "<<keys.size()<<std::endl;
        std::cout<<"weights size before: "<<weights.size()<<std::endl;
        limweight = 0;//get_weight2("--", 2, L1, bi, B);
        for(auto it = keys.begin(); it != keys.end(); ) {
//            std::cout<<"left: <"<<it->first<<"> "<<it->second.size()<<std::endl;
            if (it->second.size() < 2 || get_weight2(it->first, (it->second).size(), L1, bi, B)<=limweight) {
                weights[it->first] = 0;
//                weights.erase(it->first);
                it = keys.erase(it);
            } else {
                weights[it->first] = wei_fun(it->first, (it->second).size(), L1, bi, B);
                ++it;
            }
        }
...
//        for(auto it=sorted_by_weights.begin(); it!=sorted_by_weights.end(); it++) {
        for(auto it=sorted_by_weights.begin(); it!=sorted_by_weights.end(); ) {
            if(weights.count(it->first)) {
                it->second = weights[it->first];
                weights[it->first] = 0;
//                weights.erase(it->first);
        ...
        std::cout<<"weights size after: "<<weights.size()<<std::endl;
...
//        sorted_by_weights.reserve(sorted_by_weights.size()+weights.size());
        ...
        std::sort(sorted_by_weights.begin(), sorted_by_weights.end(), [](const auto& a, const auto& b){return a.second<b.second;});
//        std::cout<<"sorted_by_weights.back(): <"<<sorted_by_weights.back().first<<"> "<<sorted_by_weights.back().second<<std::endl;
//        to_check.clear();
        weights.clear();


        std::cout<<"new key = <"<<key2<<"> "<<key2_count<<std::endl;
        std::cout<<"keys left: "<<keys.size()<<std::endl;
        keys_stack.push_back(key2);
        bi += 1;
//        L1 -= key2.size()*keys[key2].size()+ceil(bi*keys[key2]/B);
        L1 -= key2.size()*key2_count+ceil(bi*key2_count/B);
...
//    keys_intersections.clear();
    ...
    size_t prev_pos = keys_final_positions.begin()->first;
    size_t prev_len = keys_final_positions.begin()->second;
    int dist = 0;
    size_t non_compressed_seqs_len=0, new_splitted_len=0;
    std::vector<std::string> non_compressed_seqs;
    for(auto it=keys_final_positions.begin(); it!=keys_final_positions.end(); it++) {
        dist = it->first-(prev_pos+prev_len);
        if(dist>0) {
            auto subs = text.substr(prev_pos+prev_len, dist);

            splitted.push_back(subs);
            non_compressed_seqs.push_back(subs);
            new_splitted.push_back("");
            non_compressed_seqs_len += subs.size();
...
//                }
        ...
        splitted.push_back(text.substr(it->first, it->second));
        new_splitted.push_back(text.substr(it->first, it->second));
        new_splitted_len += it->second;
        prev_pos = it->first;
        prev_len = it->second;
    }
